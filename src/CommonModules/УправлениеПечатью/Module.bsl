...
Функция НайтиМакет(ПутьКМакету, КодЯзыка, ТолькоПоставляемый = Ложь)
	...
	Попытка //DL 2023-09-09 
		...
		Исключение  
			Если НайденныйМакет <> Неопределено Тогда
				ВызватьИсключение Нстр( "ru ='Выполнение прервано, нет отмеченных строк  для печати сертификата'"); 
			Иначе 
				ВызватьИсключение Нстр( "ru ='Выполнение прервано, 
				|не выбрана строка для формирования документа word или  вид сертификата 
				|В открытом документе,установите галочку в  колонке с изображением сертификата,
				|в строке которую необходимо печатать и установите вид сертификата'");
			КонецЕсли;
		КонецПопытки; 
	//DL 2023-09-09 )	
	ИмяМакета = ЧастиПути[ЧастиПути.ВГраница()];
